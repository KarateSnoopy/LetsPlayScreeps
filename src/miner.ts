...
export class Miner
{
    public static run(room: Room, creep: Creep, rm: M.RoomMemory): void
    {
        const cm = M.cm(creep);
        ...
        else
        {
            ...
            if (!cm.gathering)
            {
                ...
                Miner.dropOffEnergy(room, creep, rm, minerTask, cm);
            }
            else
            {
                //log.info(`${M.l(cm)}is moving to mine`);
                Miner.harvestEnergy(creep, cm, rm, minerTask);
            }
        }
    }

    private static harvestEnergy(creep: Creep, cm: M.MyCreepMemory, rm: M.RoomMemory, minerTask: M.MinerTask): void
    {
        //log.info(`${M.l(cm)}is moving to mine`);
    ...
    private static buildIfCan(room: Room, creep: Creep, rm: M.RoomMemory, cm: M.MyCreepMemory): boolean
    {
        ...
        else
        {
            ...
            if (M.roomState.containers.length !== rm.containerPositions.length)
            {
                log.info(`${M.l(cm)}M.roomState.containers.length=${M.roomState.containers.length}. rm.containerPositions.length=${rm.containerPositions.length}`);
    ...
    private static dropOffEnergy(room: Room, creep: Creep, rm: M.RoomMemory, minerTask: M.MinerTask, cm: M.MyCreepMemory): void
    {
        let target: Structure | undefined;

        if (minerTask.sourceContainer === undefined ||
            rm.techLevel < 3 ||
            M.roomState.builders.length === 0)
        {
        ...
        else
        {
            if (room.controller !== undefined)
            {
                ...
                {
                    isBuilding = Miner.buildIfCan(room, creep, rm, cm);
                }
